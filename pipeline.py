#Empezamos importando las librerias que usaremos para la preparacion de los datos:
...
if __name__ == '__main__':
    ...
    #El proposito de esta seccion es importar los datasets de diferentes formatos contenidos en el 
    #directorio datasets a DataFrames de Pandas, de manera automatizada, organizandolos en diccionarios
    #para facilitar su acceso y limpieza

    print('----inicializando extraccion de datos----')
    ...
    # Creamos un diccionario con los nombres de cada archivo y su extension para los datasets iniciales y uno para los incrementales
    datasets_extensions = {}
    ...
    # Se determina si hay o no carga incremental(Este valor sera util al momento de la carga)
    if len(archivos_incrementales) > 0:
        carga_incremental = True
        print('Carga incremental detectada')
    else: 
        carga_incremental = False
        print('No se detecta carga incremental, se lleva a cabo la carga inicial')
    
    # Creamos los diccionarios para los datasets y los poblamos:
    #1. Tabla de echos inicial(ps_2020)
    #2. Tabla de echos incremental(ps_2020_i)
    #3. Dimension
    # Automarizamos la importacion de los archivos originales contenidos en el directorio "Dtasets"
